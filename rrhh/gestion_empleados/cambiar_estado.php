...
if (!isset($_GET['id']) || !is_numeric($_GET['id']) || !isset($_GET['accion'])) {
    header("Location: /rrhh/gestion_empleados/lista_empleados.php?mensaje=Parámetros inválidos&tipo=error");
    exit();
}
...
if ($accion != 'alta' && $accion != 'baja') {
    header("Location: /rrhh/gestion_empleados/lista_empleados.php?mensaje=Acción inválida&tipo=error");
    exit();
}
...
header("Location: /rrhh/gestion_empleados/lista_empleados.php?mensaje=" . urlencode($mensaje) . "&tipo=" . $tipo);
exit();
?>